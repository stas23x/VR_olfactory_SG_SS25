...
namespace TriangleNet
{
    ...
    public class Mesh : IMesh
    {
        ...
        public void Refine(QualityOptions quality, bool delaunay = false)
        {
            ...
            if (behavior.Poly) insegments = behavior.useSegments ? subsegs.Count : hullsize;

            Reset();

            if (qualityMesher == null) qualityMesher = new QualityMesher(this, new Configuration());
        ...
        public void Renumber(NodeNumbering num)
        {
            ...
            {
                id = 0;
                foreach (Vertex node in vertices.Values) node.id = id++;
            }
            else if (num == NodeNumbering.CuthillMcKee)
            {
                var rcm = new CuthillMcKee();
                int[] iperm = rcm.Renumber(this);

                // Permute the node indices.
                foreach (Vertex node in vertices.Values) node.id = iperm[node.id];
            }
            ...
            id = 0;
            foreach (Triangle item in triangles) item.id = id++;
        }
            ...
            ref Osub splitseg, bool segmentflaws, bool triflaws)
        {
            ...
            if (intersect == LocateResult.OnEdge || intersect == LocateResult.Outside)
            {
                ...
                if (splitseg.seg != null)
                {
                    ...
                    if (newvertex.label == 0) newvertex.label = splitseg.seg.boundary;
                }
                ...
                if (doflip)
                {
                    ...
                    else
                    {
                        ...
                        if (doflip)
                        {
                            ...
                            if (checkquality) flipstack.Push(horiz);
        ...
        internal void InsertSubseg(ref Otri tri, int subsegmark)
        {
            ...
            if (triorg.label == 0) triorg.label = subsegmark;
            if (tridest.label == 0) tridest.label = subsegmark;
        ...
        internal void DeleteVertex(ref Otri deltri)
        {
            ...
            if (leftsubseg.seg.hash != DUMMY) deltri.SegBond(ref leftsubseg);
            righttri.Pivot(ref rightsubseg);
            if (rightsubseg.seg.hash != DUMMY) deltriright.SegBond(ref rightsubseg);

            // Set the new origin of 'deltri' and check its quality.
            neworg = lefttri.Org();
            deltri.SetOrg(neworg);
            if (behavior.NoBisect == 0) qualityMesher.TestTriangle(ref deltri);
