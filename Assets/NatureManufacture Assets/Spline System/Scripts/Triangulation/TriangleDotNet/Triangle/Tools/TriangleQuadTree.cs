...
namespace TriangleNet.Tools
{
    ...
    public class TriangleQuadTree
    {
        ...
        public ITriangle Query(double x, double y)
        {
            ...
            List<int> indices = root.FindTriangles(point);

            foreach (int i in indices)
            {
                ITriangle tri = triangles[i];

                if (IsPointInTriangle(point, tri.GetVertex(0), tri.GetVertex(1), tri.GetVertex(2))) return tri;
            }
        ...
        private class QuadNode
        {
            ...
            public QuadNode(Rectangle box, TriangleQuadTree tree, bool init)
            {
                ...
                if (init)
                {
                    ...
                    for (int i = 0; i < count; i++) triangles.Add(i);
                }
            }
            ...
            public List<int> FindTriangles(Point searchPoint)
            {
                ...
                if (regions[region] == null) return triangles;
            ...
            private void FindTriangleIntersections(Point[] triangle, int index)
            {
                ...
                for (int i = 0; i < 3; k = i++)
                {
                    ...
                    if (dx != 0.0) FindIntersectionsWithX(dx, dy, triangle, index, k);
                    if (dy != 0.0) FindIntersectionsWithY(dx, dy, triangle, index, k);
                }
            }
            ...
            private void FindIntersectionsWithX(double dx, double dy, Point[] triangle, int index, int k)
            {
                ...
                if (t < 1 + EPS && t > -EPS)
                {
                    ...
                    if (yComponent < pivot.y && yComponent >= bounds.Bottom)
                        AddToRegion(index, SW);
                    else if (yComponent <= bounds.Top) AddToRegion(index, NW);
                }
                ...
                if (t < 1 + EPS && t > -EPS)
                {
                    ...
                    if (yComponent < pivot.y && yComponent >= bounds.Bottom)
                        AddToRegion(index, SE);
                    else if (yComponent <= bounds.Top) AddToRegion(index, NE);
                }
            }
            ...
            private void FindIntersectionsWithY(double dx, double dy, Point[] triangle, int index, int k)
            {
                ...
                if (t < 1 + EPS && t > -EPS)
                {
                    ...
                    if (xComponent > pivot.x && xComponent <= bounds.Right)
                        AddToRegion(index, SE);
                    else if (xComponent >= bounds.Left) AddToRegion(index, SW);
                }
                ...
                if (t < 1 + EPS && t > -EPS)
                {
                    ...
                    if (xComponent > pivot.x && xComponent <= bounds.Right)
                        AddToRegion(index, NE);
                    else if (xComponent >= bounds.Left) AddToRegion(index, NW);
                }
            }
            ...
            {
                int b = 2;
                if (point.y < pivot.y) b = 0;
                if (point.x > pivot.x) b++;
                return b;
            }
