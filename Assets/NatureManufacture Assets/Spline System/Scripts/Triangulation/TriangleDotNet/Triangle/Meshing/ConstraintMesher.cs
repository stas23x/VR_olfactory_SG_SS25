    ...
    internal class ConstraintMesher
    {
        ...
        public void Apply(IPolygon input, ConstraintOptions options)
        {
            ...
            if (options != null)
            {
                ...
                if (behavior.ConformingDelaunay) behavior.Quality = true;
            }
        ...
        private void FormSkeleton(IPolygon input)
        {
            ...
            if (behavior.Poly)
            {
                ...
                if (input.Segments.Count > 0) mesh.MakeVertexMap();
                ...
                if (!hulltri.IsInfected())
                {
                    ...
                    else
                    {
                        ...
                        if (hullsubseg.seg.boundary == 0)
                        {
                            ...
                            horg = hulltri.Org();
                            hdest = hulltri.Dest();
                            if (horg.label == 0) horg.label = 1;
                            if (hdest.label == 0) hdest.label = 1;
                        }
                    }
                }
                ...
                for (testtri.orient = 0; testtri.orient < 3; testtri.orient++)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            norg = neighbor.Org();
                            ndest = neighbor.Dest();
                            if (norg.label == 0) norg.label = 1;
                            if (ndest.label == 0) ndest.label = 1;
                        }
                    }
                }
        ...
        private void SegmentIntersection(ref Otri splittri, ref Osub splitsubseg, Vertex endpoint2)
        {
            ...
            if (mesh.steinerleft > 0) mesh.steinerleft--;
        ...
        private bool ScoutSegment(ref Otri searchtri, Vertex endpoint2, int newmark)
        {
                ...
                (rightvertex.x == endpoint2.x && rightvertex.y == endpoint2.y))
            {
                ...
                if (leftvertex.x == endpoint2.x && leftvertex.y == endpoint2.y) searchtri.Lprev();
        ...
        private void DelaunayFixup(ref Otri fixuptri, bool leftside)
        {
            ...
            if (fartri.tri.id == Mesh.DUMMY) return;
            neartri.Pivot(ref faredge);
            if (faredge.seg.hash != Mesh.DUMMY) return;
        ...
        private void InsertSegment(Vertex endpoint1, Vertex endpoint2, int newmark)
        {
            ...
            if (searchtri1.tri != null) checkvertex = searchtri1.Org();
            ...
            if (searchtri2.tri != null) checkvertex = searchtri2.Org();
