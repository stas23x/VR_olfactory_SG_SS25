...
namespace TriangleNet.Meshing
{
    ...
    public static class Converter
    {
        ...
        public static DcelMesh ToDCEL(Mesh mesh)
        {
            ...
            foreach (TVertex v in mesh.vertices.Values)
            {
            ...
            Dictionary<int, HalfEdge> boundary = new();
            ...
            foreach (List<HalfEdge> t in map)
            {
                edge = t[0];
                next = t[1];
            ...
            foreach (HalfEdge e in boundary.Values) e.next = boundary[e.twin.origin.id];
        ...
        private static List<Otri>[] SetNeighbors(Mesh mesh, ITriangle[] triangles)
        {
            ...
            foreach (Triangle item in mesh.triangles)
            {
                ...
                if (mesh.behavior.VarArea) tri.tri.area = triangles[i].Area;
