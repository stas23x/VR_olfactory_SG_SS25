    ...
    public class BoundedVoronoiLegacy : IVoronoi
    {
        ...
        private void Generate()
        {
            ...
            foreach (Vertex v in mesh.vertices.Values)
                if (v.type == VertexType.FreeVertex || v.label == 0)
                    ConstructCell(v);
                else if (includeBoundary) ConstructBoundaryCell(v);
            ...
            for (int i = 0; i < segPoints.Count; i++) points[length + i] = segPoints[i];
        ...
        private void TagBlindTriangles()
        {
            ...
            foreach (SubSegment ss in mesh.subsegs.Values)
            {
                ...
                if (f.tri.id != Mesh.DUMMY && !f.tri.infected) triangles.Push(f.tri);
        ...
        private void ConstructCell(Vertex vertex)
        {
            ...
            List<Point> vpoints = new();

            // Call f_init a triangle incident to x
            vertex.tri.Copy(ref f_init);

            if (f_init.Org() != vertex) throw new Exception("ConstructCell: inconsistent topology.");
        ...
        private void ConstructBoundaryCell(Vertex vertex)
        {
            ...
            if (f_init.Org() != vertex) throw new Exception("ConstructBoundaryCell: inconsistent topology.");
            // Let f be initialized to f_init
            f_init.Copy(ref f);
            // Call f_next the next triangle counterclockwise around x
            f_init.Onext(ref f_next);
        ...
        private IEnumerable<IEdge> EnumerateEdges()
        {
            ...
            foreach (VoronoiRegion region in Regions)
            {
                ...
                if (region.Bounded && first != null) edges.Add(new Edge(last.id, first.id));
            }
