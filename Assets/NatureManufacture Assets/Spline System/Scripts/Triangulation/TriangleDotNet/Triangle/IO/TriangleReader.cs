...
namespace TriangleNet.IO
{
    ...
    public class TriangleReader
    {
        ...
        public Polygon ReadNodeFile(string nodefilename, bool readElements)
        {
            ...
            {
                if (!TryReadLine(reader, out line)) throw new Exception("Can't read input file.");

                // Read number of vertices, number of dimensions, number of vertex
                // attributes, and number of boundary markers.
                invertices = int.Parse(line[0]);

                if (invertices < 3) throw new Exception("Input must have at least three input vertices.");

                if (line.Length > 1)
                    if (int.Parse(line[1]) != 2)
                        throw new Exception("Triangle only works with two-dimensional meshes.");

                if (line.Length > 2) attributes = int.Parse(line[2]);

                if (line.Length > 3) nodemarkers = int.Parse(line[3]);
                ...
                if (invertices > 0)
                    ...
                    {
                        if (!TryReadLine(reader, out line)) throw new Exception("Can't read input file (vertices).");

                        if (line.Length < 3) throw new Exception("Invalid vertex.");

                        if (i == 0) startIndex = int.Parse(line[0], nfi);
            ...
            if (readElements)
            {
                ...
                if (File.Exists(elefile)) ReadEleFile(elefile, true);
            }
        ...
        public Polygon ReadPolyFile(string polyfilename, bool readElements, bool readArea)
        {
            ...
            using (var reader = new StreamReader(polyfilename))
            {
                ...
                if (points.Count == 0) throw new Exception("No nodes available.");

                // Read the segments from a .poly file.

                // Read number of segments and number of boundary markers.
                if (!TryReadLine(reader, out line)) throw new Exception("Can't read input file (segments).");

                int insegments = int.Parse(line[0]);

                int segmentmarkers = 0;
                if (line.Length > 1) segmentmarkers = int.Parse(line[1]);
                ...
                for (int i = 0; i < insegments; i++)
                {
                    ...
                    if (line.Length < 3) throw new Exception("Segment has no endpoints.");
                    ...
                    if (segmentmarkers > 0 && line.Length > 3) mark = int.Parse(line[3]);
                ...
                // Read the holes.
                if (!TryReadLine(reader, out line)) throw new Exception("Can't read input file (holes).");
                ...
                if (holes > 0)
                    for (int i = 0; i < holes; i++)
                    {
                        ...
                        if (line.Length < 3) throw new Exception("Invalid hole.");
                ...
                if (TryReadLine(reader, out line))
                {
                    ...
                    if (regions > 0)
                        ...
                        {
                            if (!TryReadLine(reader, out line)) throw new Exception("Can't read input file (region).");

                            if (line.Length < 4) throw new Exception("Invalid region attributes.");

                            if (!int.TryParse(line[3], out id)) id = i;

                            double area = 0.0;

                            if (line.Length > 4) double.TryParse(line[4], NumberStyles.Number, nfi, out area);
            ...
            if (readElements)
            {
                ...
                if (File.Exists(elefile)) ReadEleFile(elefile, readArea);
            }
        ...
        private List<ITriangle> ReadEleFile(string elefilename, bool readArea)
        {
            ...
            using (var reader = new StreamReader(elefilename))
            {
                ...
                if (!TryReadLine(reader, out line)) throw new Exception("Can't read input file (elements).");
                ...
                if (attributes > 1) Log.Instance.Warning("Triangle attributes not supported.", "FileReader.Read");
                ...
                for (int i = 0; i < intriangles; i++)
                {
                    ...
                    if (line.Length < 4) throw new Exception("Triangle has no nodes.");
            ...
            if (readArea)
            {
                ...
                if (File.Exists(areafile)) ReadAreaFile(areafile, intriangles);
            }
        ...
        private double[] ReadAreaFile(string areafilename, int intriangles)
        {
            ...
            using (var reader = new StreamReader(areafilename))
            {
                ...
                if (!TryReadLine(reader, out line)) throw new Exception("Can't read input file (area).");
                ...
                for (int i = 0; i < intriangles; i++)
                {
                    ...
                    if (line.Length != 2) throw new Exception("Triangle has no nodes.");
        ...
        public List<Edge> ReadEdgeFile(string edgeFile, int invertices)
        {
            ...
            using (var reader = new StreamReader(edgeFile))
            {
                ...
                int edgemarkers = 0;
                if (line.Length > 1) edgemarkers = int.Parse(line[1]);

                if (inedges > 0) data = new List<Edge>(inedges);
                ...
                for (int i = 0; i < inedges; i++)
                {
                    ...
                    if (edgemarkers > 0 && line.Length > 3) mark = int.Parse(line[3]);
        ...
        private void ReadVertex(List<Vertex> data, int index, string[] line, int attributes, int marks)
        {
            ...
            if (marks > 0 && line.Length > 3 + attributes) v.Label = int.Parse(line[3 + attributes]);
        ...
        public void Read(string filename, out Polygon geometry, out List<ITriangle> triangles)
        {
            ...
            if (File.Exists(path) && geometry != null) triangles = ReadEleFile(path);
        }
