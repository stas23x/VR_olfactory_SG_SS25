...
namespace TriangleNet.IO
{
    ...
    public class TriangleWriter
    {
        ...
        private void WriteNodes(StreamWriter writer, Mesh mesh)
        {
            ...
            if (behavior.Jettison) outvertices = mesh.vertices.Count - mesh.undeads;
            ...
            if (writer != null)
            {
                ...
                else
                {
                    ...
                    foreach (Vertex node in mesh.vertices.Values) nodes[node.id] = node;
...
// Write attributes.
        ...
        public void WriteElements(Mesh mesh, string filename)
        {
            ...
            using (var writer = new StreamWriter(filename))
            {
                ...
                foreach (Triangle item in mesh.triangles)
                {
                    ...
                    if (regions) writer.Write(" {0}", tri.tri.label);
        ...
        public void WritePoly(IPolygon polygon, string filename)
        {
            ...
            using (var writer = new StreamWriter(filename))
            {
                ...
                foreach (Point hole in polygon.Holes) writer.WriteLine("{0} {1} {2}", j++, hole.X.ToString(nfi), hole.Y.ToString(nfi));
        ...
        public void WritePoly(Mesh mesh, string filename, bool writeNodes)
        {
            ...
            using (var writer = new StreamWriter(filename))
            {
                ...
                foreach (Point hole in mesh.holes) writer.WriteLine("{0} {1} {2}", j++, hole.X.ToString(nfi), hole.Y.ToString(nfi));
